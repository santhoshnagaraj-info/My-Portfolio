...
export default function Skills() {
  return (
    <section id="skills" className="px-[10%] py-[4%] w-full h-screen-[50%] bg-transparent items-center grid grid-cols-1 backdrop-blur-md border border-gray-400">
        <p className="p-3 text-green-600 font-semibold text-sm gap-[3%] uppercase">Technical Skills</p>
        <h2 className="p-3 text-[200%] lg:text-[300%] font-bold text-gray-900"> Technologies I work with </h2>

        <div className="p-5 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-8">
          {techStackData.map((stack) => {
            const Icon = iconMap[stack.icon as keyof typeof iconMap];
            return (
              <div
                key={stack.id} className="bg-white border border-gray-100 rounded-xl p-6 flex flex-col items-center text-center shadow-md hover:scale-105 transition-shadow">
